...
def query_macro_days_1(start_time, end_time):
    ...
    while True:
        ...
        query = """
        SELECT updated, viewer_uid, keyword
        ...
        if query_count == 1:
            result.to_csv('csv/item_search_02.csv', index=False, mode='w', header=True)

        else:
            result.to_csv('csv/item_search_02.csv', index=False, mode='a', header=False)
